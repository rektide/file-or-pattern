...
pub struct StamperOptions<'a> {
    /// Name of the processor to use for stamping (if set, skips processor argument)
...
pub struct StamperHandle<T> {
    ...
    /// Sender used to resolve the promise
...
impl<T> StamperHandle<T> {
    ...
    ///
    /// Returns Ok(()) if the promise was successfully resolved,
    /// or Err(value) if the receiver was already dropped.
...
/// the StamperHandle promise-like interface.
